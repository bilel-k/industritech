...
for svc in iot-mosquitto iot-nodered iot-influxdb iot-grafana iot-webapp iot-sensor-sim; do
...
check_port() {
  ...
  if retry 25 1 nc -z -w1 "$host" "$port"; then
...
check_port "Webapp"            localhost 8080
...
# ── 5b. Webapp HTTP ─────────────────────────────────────────────────────────
echo -e "\n-- Webapp --"
WEBAPP_HOME=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/)
[ "$WEBAPP_HOME" = "200" ] && ok "Webapp home OK (200)" || fail "Webapp home KO ($WEBAPP_HOME)"

CHAT_STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/api/chat/status)
[ "$CHAT_STATUS_CODE" = "200" ] && ok "Chat status endpoint OK (200)" || fail "Chat status endpoint KO ($CHAT_STATUS_CODE)"

CHAT_STATUS_ENABLED=$(curl -s http://localhost:8080/api/chat/status | python3 -c "import sys,json; d=json.load(sys.stdin); print('true' if bool(d.get('enabled')) else 'false')" 2>/dev/null || echo "parse_error")
if [ "$CHAT_STATUS_ENABLED" = "true" ] || [ "$CHAT_STATUS_ENABLED" = "false" ]; then
  ok "Chat status JSON parse OK"
else
  fail "Chat status JSON parse KO"
fi
